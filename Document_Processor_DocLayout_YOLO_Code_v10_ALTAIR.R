...
Sys.setenv(RETICULATE_PYTHON = "/home/altair-lab/miniconda3/envs/PDF-OCR/bin/python")

simple_process_document <- function(pdf_path) {
  message(glue("Processing PDF: {pdf_path}"))
  
  # Setup
  use_python("/home/altair-lab/miniconda3/envs/PDF-OCR/bin/python", required = TRUE)
  ...
  dlyolo <- reticulate::import("doclayout_yolo")
  YOLOv10 <- dlyolo$YOLOv10
  
  model <-  YOLOv10("/home/altair-lab/Desktop/MINER_DB/doclayout_yolo_docstructbench_imgsz1024.pt")
...
for i, (x, y, w, h) in enumerate(boxes):
    try:
      ...
      if (length(ocr_results) > 0) {
        ...
        df <- sort_by_reading_order_universal_stdaln(df, page_num)
...
sort_by_reading_order_universal_stdaln <- function(df, page_num) {
    ...
    if (page_num == 1) {
      ...
      is_elsevier_format <- any(str_detect(tolower(df$text), "elsevier|sciencedirect|computational.*structural.*biotechnology"))
      
      
      if (is_elsevier_format) {
        message("  → Elsevier format detected")
        
        # Elsevier page 1 has special layout
        df <- df %>%
          mutate(
            is_header = y < 300,  # Journal name area
            is_article_type = y >= 300 & y < 400,  # "Review Article" label
            is_title = y >= 400 & y < 700 & !str_detect(tolower(text), "journal|elsevier"),
            is_authors = y >= 700 & y < 1000 & str_detect(text, "[A-Z][a-z]+"),
            is_article_info = x < 800 & y >= 1000 & y < 1600,  # Left column info box
            is_abstract = x >= 800 & y >= 1000 & y < 1600,  # Right column abstract
            is_main_content = y >= 1600  # Two-column main text
          )
        
        # Process each section
        header <- df %>% filter(is_header) %>% arrange(y, x)
        article_type <- df %>% filter(is_article_type) %>% arrange(y, x)
        title <- df %>% filter(is_title) %>% arrange(y, x)
        authors <- df %>% filter(is_authors) %>% arrange(y, x)
        article_info <- df %>% filter(is_article_info) %>% arrange(y, x)
        abstract <- df %>% filter(is_abstract) %>% arrange(y, x)
        
        # Main content with column detection
        main_content <- df %>% filter(is_main_content)
        
        if (nrow(main_content) > 0) {
          layout_info <- detect_page_layout_universal_stdaln(main_content)
        ...
        result <- bind_rows(header, article_type, title, authors, article_info, abstract, main_content) %>%
          select(-x_center, -y_center, -area, -original_order,
                 -is_header, -is_article_type, -is_title, -is_authors, 
                 -is_article_info, -is_abstract, -is_main_content)
        
        message(glue("  → Elsevier layout complete"))
      ...
      layout_info <- detect_page_layout_universal_stdaln(df)
...
detect_page_layout_universal_stdaln <- function(df) {
...
GenericDocumentProcessor <- R6::R6Class("GenericDocumentProcessor",
                                        ...
                                        public = list(
                                          ...
                                          detect_document_format = function(raw_df) {
                                            ...
                                            # Elsevier detection
                                            if (str_detect(publisher_text, "elsevier|sciencedirect|computational.*structural.*biotechnology")) {
                                              format_scores$elsevier <- format_scores$elsevier + 5
                                            }
                                            if (str_detect(publisher_text, "journal homepage.*www\\.elsevier\\.com")) {
                                              format_scores$elsevier <- format_scores$elsevier + 3
                                            }
                                            if (str_detect(publisher_text, "review article|research article|article info")) {
                                              format_scores$elsevier <- format_scores$elsevier + 2
                                            }
                                            # Look for typical Elsevier layout markers
                                            if (str_detect(all_text, "article\\s+info|abstract.*keywords")) {
                                              format_scores$elsevier <- format_scores$elsevier + 2
                                            }
...
")
                                          ...
                                          extract_authors_with_llm = function(raw_df, structure_analysis, model) {
                                            ...
                                            is_jsm_format <- any(str_detect(tolower(page1_sorted$text), "jsm nanotechnology"))
                                            is_nature_format <- any(str_detect(tolower(page1_sorted$text), "nature"))
                                            is_elsevier_format <- any(str_detect(tolower(page1_sorted$text), "elsevier|sciencedirect"))
                                            
                                            # Apply reading order
                                            layout_info <- detect_page_layout_universal_stdaln(page1_sorted)
                                            if (layout_info$columns == 2) {
                                              message(glue("  → Two columns detected: gap {round(layout_info$boundary)}px"))
                                            ...
                                            if (is_elsevier_format) {
                                              message("  → Using Elsevier format author detection")
                                              
                                              # Elsevier authors are typically between y=700-1000 on page 1
                                              # and contain person names (not departments/universities)
                                              elsevier_author_candidates <- page1_sorted %>%
                                                filter(
                                                  y >= 700 & y < 1000,  # Author area
                                                  # MUST have actual person name patterns (First Last format)
                                                  str_detect(text, "[A-Z][a-z]+\\s+[A-Z][a-z]+"),
                                                  # MUST NOT be primarily institutional text
                                                  !str_detect(text, "^\\*?\\s*(Department|Faculty|Institute|Laboratory|School|College|Hospital)"),
                                                  # Length check
                                                  nchar(text) > 30 & nchar(text) < 500
                                                ) %>%
                                                mutate(
                                                  # Enhanced scoring for Elsevier
                                                  person_name_count = str_count(text, "\\b[A-Z][a-z]+\\s+[A-Z][a-z]+\\b"),
                                                  has_name_separators = str_detect(text, ",|;"),
                                                  has_affiliation_markers = str_detect(text, "[a-z°*†‡§¶#¹²³⁴⁵⁶⁷⁸⁹⁰]"),
                                                  starts_with_name = str_detect(text, "^[A-Z][a-z]+"),
                                                  
                                                  # Penalize if looks like affiliation
                                                  looks_like_affiliation = str_detect(text, "Department|University|Hospital|Institute|Laboratory|^\\*"),
                                                  
                                                  author_score = 
                                                    person_name_count * 5 +  # High weight for person names
                                                    as.numeric(has_name_separators) * 3 +
                                                    as.numeric(has_affiliation_markers) * 2 +
                                                    as.numeric(starts_with_name) * 2 -
                                                    as.numeric(looks_like_affiliation) * 10  # Heavy penalty for affiliations
                                                ) %>%
                                                filter(author_score > 5) %>%
                                                arrange(desc(author_score), y)
                                              
                                              if (nrow(elsevier_author_candidates) > 0) {
                                                message("Elsevier author candidates found:")
                                                elsevier_author_candidates %>%
                                                  mutate(preview = str_trunc(text, 100)) %>%
                                                  select(y, author_score, preview) %>%
                                                  head(3) %>%
                                                  print()
                                                
                                                authors_text <- elsevier_author_candidates %>%
                                                  slice(1) %>%
                                                  pull(text)
                                                
                                                message(glue("Selected: {str_trunc(authors_text, 100)}"))
                                                
                                              } else {
                                                message("No Elsevier author candidates found, using fallback")
                                            ...
                                            if (!str_detect(authors_text, "Authors not found") && nchar(authors_text) > 10) {
                                              ...
                                              # Pre-clean the text to help the LLM
                                              authors_text_clean <- authors_text %>%
                                                str_replace_all("[®™©]", "") %>%  # Remove registration marks
                                                str_replace_all("['']", "") %>%   # Remove various apostrophes
                                                str_replace_all("[‘’']", "") %>% 
                                                str_replace_all("[“”\"]", "") %>%   # Remove smart quotes
                                                str_replace_all("\\d+", "") %>%   # Remove numbers
                                          ...
                                          extract_abstract_universal = function(raw_df, structure_analysis, model) { 
                                            ...
                                            # Always create text_clean early and handle missing columns safely
                                            ...
                                            is_elsevier_format <- str_detect(text_for_detection, "elsevier|sciencedirect|computational.*structural.*biotechnology")
                                            
                                            message(glue("Format detection: Nature={is_nature_format}, JAC={is_jac_format}, JSM={is_jsm_format}, Elsevier={is_elsevier_format}"))
                                            ...
                                            # ELSEVIER FORMAT
                                            if (is_elsevier_format) {
                                              message("Using Elsevier format abstract extraction")
                                              return(self$extract_abstract_elsevier(raw_df))
                                            }
                                          ...
                                          extract_main_text_smart = function(raw_df, extracted_abstract = NULL) {
                                            ...
                                            # Check format
                                            if (!is.null(doc_format)) {
                                              is_mdpi <- doc_format == "mdpi" || 
                                                any(str_detect(tolower(raw_df$text), "mdpi"))
                                              is_elsevier <- doc_format == "elsevier" || 
                                                any(str_detect(tolower(raw_df$text), "elsevier|sciencedirect"))
                                            }
                                            ...
                                            if (is_elsevier) {
                                              message("Elsevier format detected - using specialized extraction")
                                              return(self$extract_main_text_elsevier(raw_df))
                                            }
                                            ...
                                            all_text <- raw_df %>%
                                              ...
                                              group_modify(~ {
                                                ...
                                                layout_info <- self$detect_page_layout_universal(page_df)
                                            ...
                                            # FIXED: Return the actual content instead of empty strings
                                            return(list(
                                              main = main_content,
                                          ...
                                          extract_main_text_mdpi_simplified = function(raw_df) {
                                            ...
                                            main_content_candidates <- page1_data %>%
                                              filter(str_detect(text_lower, "abstract")) %>%
                                          ...
                                          extract_abstract_elsevier = function(raw_df) {
                                            message("=== Elsevier Format Abstract Detection ===")
                                            ...
                                            # Elsevier typically has abstract in right column after "ABSTRACT" header
                                            # Look for the ABSTRACT header first
                                            abstract_header <- page1_data %>%
                                              filter(str_detect(toupper(text_clean), "^ABSTRACT\\s*$|^A\\s*B\\s*S\\s*T\\s*R\\s*A\\s*C\\s*T")) %>%
                                              arrange(y) %>%
                                              slice(1)
                                            
                                            if (nrow(abstract_header) > 0) {
                                              abstract_y_start <- abstract_header$y[1]
                                              abstract_x_zone <- abstract_header$x[1]
                                              
                                              message(glue("Found ABSTRACT header at y={abstract_y_start}, x={abstract_x_zone}"))
                                              
                                              # Get text below the abstract header in the same column
                                              abstract_candidates <- page1_data %>%
                                                filter(
                                                  y > abstract_y_start,
                                                  y < abstract_y_start + 800,  # Abstract typically within 800px
                                                  abs(x - abstract_x_zone) < 200,  # Same column
                                                  nchar(text_clean) > 50,
                                                  !str_detect(tolower(text_clean), "keywords?:|article info")
                                                ) %>%
                                                arrange(y)
                                              ...
                                              if (nrow(abstract_candidates) > 0) {
                                                ...
                                                message(glue("Extracted abstract: {nchar(abstract_text)} chars"))
                                                return(abstract_text)
                                              }
                                            }
                                            
                                            # Fallback: look for substantial text in right column area
                                            right_column_text <- page1_data %>%
                                              filter(
                                                x > 1000,  # Right side of page (adjust based on your DPI)
                                                y > 600 & y < 1400,
                                                nchar(text_clean) > 100,
                                                !str_detect(tolower(text_clean), "keywords?:|article info|doi:")
                                              ) %>%
                                              arrange(y)
                                            
                                            if (nrow(right_column_text) > 0) {
                                              abstract_text <- right_column_text %>%
                                                slice(1:2) %>%  # Take first couple of regions
                                                pull(text_clean) %>%
                                                paste(collapse = " ")
                                          ...
                                          extract_title_elsevier = function(raw_df) {
                                            message("=== Elsevier Format Title Detection ===")
                                            ...
                                            # Exclude journal name and metadata
                                            title_candidates <- page1_data %>%
                                              filter(
                                                # Title is usually between y=400-800 for Elsevier
                                                y > 400 & y < 800,
                                                nchar(text_clean) > 30 & nchar(text_clean) < 400,
                                                # Exclude obvious non-title content
                                                !str_detect(text_lower, "journal|elsevier|doi:|review article|research article|sciencedirect"),
                                                !str_detect(text_lower, "^(article|paper|correspondence)\\s*$"),
                                                # Positive indicators
                                                str_detect(text_clean, "^[A-Z].*[a-z]"),  # Starts with capital
                                                str_count(text_clean, "\\s") > 3  # Multiple words
                                              ) %>%
                                              arrange(y)
                                            
                                            if (nrow(title_candidates) > 0) {
                                              # Often the title is in multiple lines, combine them
                                              title_y_start <- title_candidates$y[1]
                                              title_regions <- title_candidates %>%
                                                filter(y < title_y_start + 200) %>%  # Within 200px of first title line
                                                pull(text_clean)
                                              
                                              title_text <- paste(title_regions, collapse = " ") %>%
                                                str_replace_all("\\s+", " ") %>%
                                                str_trim()
                                              
                                              message(glue("Found title: {str_trunc(title_text, 80)}"))
                                              return(title_text)
                                            }
                                          ...
                                          extract_main_text_elsevier = function(raw_df) {
                                            message("=== Elsevier Format Main Text Extraction ===")
                                            
                                            # Apply reading order with special handling for Elsevier's layout
                                            all_text <- raw_df %>%
                                              filter(element_type %in% c("plain text", "title") | is.na(element_type)) %>%
                                              filter(!is.na(text), nchar(str_trim(text)) > 0) %>%
                                              group_by(page) %>%
                                              group_modify(~ {
                                                ...
                                                if (page_num == 1) {
                                                  # Special handling for Elsevier page 1
                                                  # Left column has ARTICLE INFO, right column has ABSTRACT and start of content
                                                  page_df <- page_df %>%
                                                    mutate(
                                                      x_center = x + w/2,
                                                      text_clean = str_replace_all(text, "\\s+", " ") %>% str_trim(),
                                                      # Classify regions
                                                      is_article_info = str_detect(tolower(text_clean), "article info|keywords"),
                                                      is_main_content = y > 1400  # Main content starts lower on page 1
                                                    )
                                                  
                                                  # Sort with column awareness but keep article info separate
                                                  article_info <- page_df %>% filter(is_article_info) %>% arrange(y, x)
                                                  main_content <- page_df %>% filter(!is_article_info)
                                                  
                                                  # Apply two-column sorting to main content
                                                  if (nrow(main_content) > 0) {
                                                    layout_info <- self$detect_page_layout_universal(main_content)
                                                    if (layout_info$columns == 2) {
                                                      main_content <- main_content %>%
                                                        mutate(column = ifelse(x_center < layout_info$boundary, 1, 2)) %>%
                                                        arrange(column, y, x) %>%
                                                  ...
                                                  # Combine: article info first (for completeness), then main content
                                                  page_df <- bind_rows(article_info, main_content) %>%
                                                    select(-x_center, -text_clean, -is_article_info, -is_main_content)
                                                  
                                                } else {
                                                  # Regular two-column layout for other pages
                                                  page_df <- page_df %>%
                                                    mutate(x_center = x + w/2)
                                            ...
                                            # Find introduction start (skip article info box)
                                            intro_start <- all_text %>%
                                              filter(
                                                !str_detect(text_lower, "article info|keywords"),
                                                str_detect(text_clean, "^1\\.\\s+Introduction|^Introduction$|^1\\. Introduction") 
                                              ) %>%
                                              arrange(row_id) %>%
                                              slice(1) %>%
                                              pull(row_id)
                                            ...
                                            if (length(intro_start) == 0) {
                                              # Fallback: skip article info box content
                                              intro_start <- all_text %>%
                                                filter(!str_detect(text_lower, "article info|keywords|received.*revised.*accepted")) %>%
                                                filter(nchar(text_clean) > 100, page == 1) %>%
                                                arrange(row_id) %>%
                                                slice(3) %>%  # Skip first few regions
                                                pull(row_id)
                                            }
                                            
                                            # Find end (before references or author contributions)
                                            end_markers <- all_text %>%
                                              filter(
                                                str_detect(text_clean, "^References\\s*$|^Acknowledgments?|^Author\\s+statement|^CRediT\\s+authorship|^Declaration\\s+of\\s+Competing|^Compliance\\s+with\\s+ethics")
                                              ) %>%
                                              arrange(row_id) %>%
                                              slice(1) %>%
                                              pull(row_id)
                                            ...
                                            if (length(intro_start) > 0) {
                                              main_content <- all_text %>%
                                                slice(intro_start[1]:end_idx) %>%
                                                # Remove article info if it leaked through
                                                filter(!str_detect(text_lower, "^keywords:|^article info|^abbreviations")) %>%
                                                pull(text_clean) %>%
                                                paste(collapse = " ")
                                            } else {
                                              main_content <- all_text %>%
                                                slice(10:end_idx) %>%  # Skip first 10 regions as fallback
                                                pull(text_clean) %>%
                                                paste(collapse = " ")
                                            }
                                            ...
                                            # Clean up
                                            main_content <- main_content %>%
                                              str_replace_all("\\[\\d+\\]", "") %>%  # Remove citations
                                              str_replace_all("\\s+", " ") %>%
                                              str_trim()
                                            ...
                                            message(glue("Elsevier extraction complete: {nchar(main_content)} chars"))
                                            
                                            return(list(
                                              main = main_content,
                                              original = original_content,
                                              boundaries = list(start = intro_start, end = end_idx)
                                            ))
                                          },
                                          ...
                                          sort_by_reading_order_universal = function(df, page_num) {
                                            ...
                                            if (page_num == 1) {
                                              ...
                                              # Regular column detection for pages 2+
                                              layout_info <- self$detect_page_layout_universal(df)
                                          ...
                                          detect_page_layout_universal = function(df) {
                                            # Ensure x_center exists
                                            if (!"x_center" %in% names(df)) {
                                              df <- df %>%
                                                mutate(x_center = x + w/2)
                                            }
                                          ...
                                          debug_reading_order = function(summary, page_num = 1) {
                                            ...
                                            sorted_data <- self$sort_by_reading_order_universal(page_data, page_num)
...
# RUN THE CODE
setwd("/home/altair-lab/Desktop/Test")
list.files(pattern = "pdf")

pdf <- "prister-et-al-2020-altering-the-neisseria-gonorrhoeae-pile-guanine-quadruplex-loop-bases-affects-pilin-antigenic.pdf"
...
claude_text_cleaned <- processor$prepare_for_claude(summary, body_only = FALSE, 
...
$authors
[1] "Lauren L. Prister; Shaohui Yin; Laty A. Cahoon"

$abstract
[1] "Neisseria gonorrhoeae possesses a programmed recombination system that allows the bacteria to alter the major subunit of the type IV pilus, pilin or PilE. An alternate DNA structure known as a guanine quadruplex (G4) is required for pilin antigenic variation (pilin Av). The G-C base pairs within the G4 motif are required for pilin Av, but simple mutation of the loop bases does not affect pilin Av. We show that more substantial changes to the loops, in both size and nucleotide composition, with the core guanines unchanged, decrease or abrogate pilin Av. We investigated why these loop changes might influence the efficiency of pilin Av. RecA is a recombinase required for pilin Av that can bind the pilE G4 in vitro. RecA binds different G4 structures with altered loops with varied affinities. However, changes in RecA binding affinities to the loop mutants do not absolutely correlate with the pilin Av phenotypes. Interestingly, the yeast RecA ortholog, RadS1, also binds the pilE G4 structure with a higher affinity than it binds single-stranded DNA, suggesting that RecA G4 binding is conserved in eukaryotic orthologs. The thermal stability the pilE G4 structure and"

$abstract_cleaned
[1] "Neisseria gonorrhoeae possesses a programmed recombination system that allows the bacteria to alter the major subunit of the type IV pilus, pilin or PilE. An alternate DNA structure known as a guanine quadruplex (G4) is required for pilin antigenic variation (pilin Av). The G-C base pairs within the G4 motif are required for pilin Av, but simple mutation of the loop bases does not affect pilin Av. We show that more substantial changes to the loops, in both size and nucleotide composition, with the core guanines unchanged, decrease or abrogate pilin Av. We investigated why these loop changes might influence the efficiency of pilin Av. RecA is a recombinase required for pilin Av that can bind the pilE G4 in vitro. RecA binds different G4 structures with altered loops with varied affinities. However, changes in RecA binding affinities to the loop mutants do not absolutely correlate with the pilin Av phenotypes. Interestingly, the yeast RecA ortholog, RadS1, also binds the pilE G4 structure with a higher affinity than it binds single-stranded DNA, suggesting that RecA G4 binding is conserved in eukaryotic orthologs. The thermal stability the pilE G4 structure and"

$main_text
[1] "Pilin Av also depends on the activity of a promoter located adjacent to the G4-forming sequence that initiates tran- scription within the G4 sequence, resulting in a small, noncoding RNA that can only function in cis.** Transcription of the sRNA opens the DNA duplex and allows for formation of the G4 structure.°> However, we are not sure how the pilE G4 specifically functions to initiate pilin Av. Substitution of the G4-forming sequence with an I-Scel cut does stimulate recombination in the area when cut with the I-Scel enzyme but does not allow pilin Av.’? Many researchers have determined the biochemical parameters of G4 structures like folding, stability, and protein-binding partners.''*°~** The pilE G4 sequence from S’ to 3’ is GGGTGGGTTGGGTGGGG with the loop bases underlined. Interestingly, replacing the parental pilE G4 with a few different G4 motifs prevented pilin Av.” These alternate G4 motifs also had four guanine triplets, but instead of the 1:2:1 configuration, the loops were two or three bases long. Therefore, we sought to understand how the G4 loop sequences might alter pilin Av. Kuryavyi et al. reported that RecA binds the pilE G4 with micromolar affinity and can stimulate strand exchange when a G4 structure is on a DNA substrate.*’ RecA is involved in both double-strand break repair and single-strand gap repair as it binds single-stranded DNA (ssDNA) resected from double- strand breaks or single-stranded gaps. Using the ssDNA as a template, it then searches for homology within the double- stranded genome to initiate repair by mediating strand exchange.” RecA and its homologues are ubiquitous in all domains of life.*° Double-strand breaks or single-stranded gaps can form from replication fork stalling at the G4 site” Because RecA plays multiple roles in DNA recombination, in addition to G4 binding, it is unknown which of these functions are important during pilin Av. In this study, we sought to understand the size and sequence requirements of G4 l ... <truncated>

$main_text_cleaned
[1] "Pilin Av also depends on the activity of a promoter located adjacent to the G4-forming sequence that initiates transcription within the G4 sequence, resulting in a small, noncoding RNA that can only function in cis. Transcription of the sRNA opens the DNA duplex and allows for formation of the G4 structure.> However, we are not sure how the pilE G4 specifically functions to initiate pilin Av. Substitution of the G4-forming sequence with an I-Scel cut does stimulate recombination in the area when cut with the I-Scel enzyme but does not allow pilin Av.'? Many researchers have determined the biochemical parameters of G4 structures like folding, stability, and protein-binding partners.''*~** The pilE G4 sequence from S' to 3' is GGGTGGGTTGGGTGGGG with the loop bases underlined. Interestingly, replacing the parental pilE G4 with a few different G4 motifs prevented pilin Av.\" These alternate G4 motifs also had four guanine triplets, but instead of the 1:2:1 configuration, the loops were two or three bases long. Therefore, we sought to understand how the G4 loop sequences might alter pilin Av. Kuryavyi et al. reported that RecA binds the pilE G4 with micromolar affinity and can stimulate strand exchange when a G4 structure is on a DNA substrate.*' RecA is involved in both double-strand break repair and single-strand gap repair as it binds single-stranded DNA (ssDNA) resected from doublestrand breaks or single-stranded gaps. Using the ssDNA as a template, it then searches for homology within the doublestranded genome to initiate repair by mediating strand exchange.\" RecA and its homologues are ubiquitous in all domains of life. Double-strand breaks or single-stranded gaps can form from replication fork stalling at the G4 site\" Because RecA plays multiple roles in DNA recombination, in addition to G4 binding, it is unknown which of these functions are important during pilin Av. In this study, we sought to understand the size and sequence requirements of G4 loops for  ... <truncated>

$main_text_original
[1] "Lauren L. Prister, Shaohui Yin, Laty A. Cahoon, and H Steven Seifert” Cite This: Biochemistry 2020, 59, 1104-1112 Cite This: Biochemistry 2020, 59, 1104-1112 lil Metrics & More ABSTRACT: Neisseria gonorrhoeae possesses a programmed recombination system that allows the bacteria to alter the major subunit of the type IV pilus, pilin or PilE. An alternate DNA structure known as a guanine quadruplex (G4) is required for pilin antigenic variation (pilin Av). The G-C base pairs within the G4 motif are required for pilin Av, but simple mutation of the loop bases does not affect pilin Av. We show that more substantial changes to the loops, in both size and nucleotide composition, with the core guanines unchanged, decrease or abrogate pilin Av. We investigated why these loop changes might influence the efficiency of pilin Av. RecA is a recombinase required for pilin Av that can bind the pilE G4 in vitro. RecA binds different G4 structures with altered loops with varied affinities. However, changes in RecA binding affinities to the loop mutants do not absolutely correlate with the pilin Av phenotypes. Interestingly, the yeast RecA ortholog, RadS1, also binds the pilE G4 structure with a higher affinity than it binds single-stranded DNA, suggesting that RecA G4 binding is conserved in eukaryotic orthologs. The thermal stability the pilE G4 structure and guanine quadruplex (G4) is a non-B form of DNA formed using Hoogsteen base pairing from single- stranded DNA.’ A G4 motif requires at least four tracts of two or more guanines. Often there is at least one loop base between each tract, but recent findings have indicated that this is not a rule.’ Each guanine interacts with two other guanines through Hoogsteen base pairing, creating a negatively charged channel within the center of the tetrad that is stabilized by a cation.*° Duplexed DNA must first have the hydrogen bonds broken before G4 formation, and this can occur through the helicase activities necessary for transcript ... <truncated>
  
  $extraction_boundaries
$extraction_boundaries$start
[1] 15

$extraction_boundaries$end
[1] 88
...
$raw_data
# A tibble: 99 × 7
page     x     y     w     h text                                                                                                         element_type
<int> <int> <int> <int> <int> <chr>                                                                                                        <chr>       
  1     1   252   157   675   128 "Bichemistry\n"                                                                                              abandon     
2     1  2199   379   153    51 "| Article\n"                                                                                                abandon     
3     1   246   389   375    36 "pubs.acs.org/biochemistry\n"                                                                                abandon     
4     1   248   490  2067   152 "Altering the Neisseria gonorrhoeae pilE Guanine Quadruplex Loop\nBases Affects Pilin Antigenic Variation\n" title       
5     1   247   663  1459    62 "Lauren L. Prister, Shaohui Yin, Laty A. Cahoon, and H Steven Seifert”\n"                                    plain text  
6     1   344   778   639    43 "Cite This: Biochemistry 2020, 59, 1104-1112\n"                                                              plain text  
7     1  1436   778   209    42 "Read Online\n"                                                                                              plain text  
8     1   343   779   640    43 "Cite This: Biochemistry 2020, 59, 1104-1112\n"                                                              plain text  
9     1   622   920   314    52 "lil Metrics & More\n"                                                                                       title       
10     1  1169   920   473    50 "Article Recommendations\n"                                                                                  title       
# ℹ 89 more rows
