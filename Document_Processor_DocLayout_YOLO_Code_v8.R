...
")
                                          ...
                                          extract_main_text_smart = function(raw_df, extracted_abstract = NULL) {
                                            ...
                                            all_text <- raw_df %>%
                                              ...
                                              # CRITICAL: Filter out rows with NA or empty text early
                                              ...
                                              # Additional safety filter after processing
                                              filter(!is.na(text_clean), nchar(text_clean) > 0)
                                            
                                            message(glue("Sorted text: {nrow(all_text)} regions"))
                                            
                                            if (nrow(all_text) == 0) {
                                              message("ERROR: No valid text regions found after filtering")
                                              return(list(main = "", original = "", boundaries = list(start = 1, end = 1)))
                                            }
                                            
                                            # ENHANCED STRATEGY: Look for introduction first, then validate with abstract
                                            start_idx <- NULL
                                            
                                            # Step 1: Find clear introduction patterns with NA safety
                                            intro_candidates <- all_text %>%
                                              filter(!is.na(text_clean), !is.na(text_lower)) %>%
                                              mutate(
                                                # Nature format specific patterns
                                                has_nature_intro = str_detect(text_clean, "Silver nanoparticles \\(NPs\\) and nanocomposites are considered promising"),
                                                has_general_intro = str_detect(text_lower, "silver.*nanoparticles.*nanomaterials.*medical.*applications"),
                                                has_intro_header = str_detect(text_lower, "^introduction\\s*$"),
                                                ...
                                                not_front_matter = !str_detect(text_lower, "department|university|@|email|doi|copyright|keywords"),
                                                
                                                # Enhanced scoring with Nature-specific patterns
                                                intro_score = as.numeric(has_nature_intro) * 10 +  # Very strong Nature intro pattern
                                                  as.numeric(has_general_intro) * 6 +
                                                  as.numeric(has_intro_header) * 2 +
                                                  as.numeric(starts_with_number) * 2 +
                                                  as.numeric(is_substantial) * 1 +
                                                  as.numeric(starts_properly) * 1 +
                                                  as.numeric(not_front_matter) * 1
                                              ) %>%
                                              filter(intro_score >= 6) %>%
                                              arrange(desc(intro_score), row_id)
                                            
                                            message("Introduction candidates:")
                                            if (nrow(intro_candidates) > 0) {
                                              intro_candidates %>%
                                                mutate(preview = str_trunc(text_clean, 80)) %>%
                                                select(row_id, intro_score, preview) %>%
                                                slice(1:3) %>%
                                                print()
                                            }
                                            ...
                                            # Step 2: Abstract boundary method with NA safety
                                            if (is.null(start_idx) && !is.null(extracted_abstract) && 
                                                !is.na(extracted_abstract) && nchar(extracted_abstract) > 50 && 
                                              ...
                                              if (length(abstract_words) >= 8) {
                                                ...
                                                for (i in 1:min(30, nrow(all_text))) {
                                                  ...
                                                  # NA safety check
                                                  if (!is.na(text_to_check) && nchar(text_to_check) > 100 && 
                                                      str_detect(text_to_check, fixed(search_phrase))) {
                                                    abstract_location <- i
                                              ...
                                              if (!is.null(abstract_location)) {
                                                for (j in (abstract_location + 1):min(nrow(all_text), abstract_location + 15)) {
                                                  if (j <= nrow(all_text)) {
                                                    ...
                                                    # NA safety check
                                                    if (!is.na(next_text) && nchar(next_text) > 100 && 
                                            ...
                                            # Step 3: Final fallback with NA safety
                                            if (is.null(start_idx)) {
                                              ...
                                              for (i in 10:min(50, nrow(all_text))) {
                                                if (i <= nrow(all_text)) {
                                                  ...
                                                  # Comprehensive NA safety check
                                                  if (!is.na(text_check) && !is.na(text_lower) && 
                                                      nchar(text_check) > 150 && 
                                            ...
                                            # Find end boundary with COMPREHENSIVE NA handling
                                            end_indicators <- c(
                                            ...
                                            search_start <- min(search_start, nrow(all_text))  # Safety check
                                            
                                            for (i in search_start:nrow(all_text)) {
                                              text_check <- all_text$text_lower[i]
                                              text_clean <- all_text$text_clean[i]
                                              
                                              # CRITICAL FIX: Add comprehensive NA checks
                                              if (!is.na(text_check) && !is.na(text_clean)) {
                                                
                                                # Check for end indicators with NA safety
                                                end_matches <- sapply(end_indicators, function(pattern) {
                                                  !is.na(text_check) && str_detect(text_check, pattern)
                                                })
                                                
                                                if (any(end_matches, na.rm = TRUE)) {
                                                  end_idx <- i - 1
                                                  message(glue("Found end section at row {i}: {str_trunc(text_clean, 60)}"))
                                                  break
                                                }
                                                
                                                # Reference pattern check with NA safety
                                                if (str_detect(text_clean, "^\\d+\\.\\s.*et al\\.|journal|proc\\.") &&
                                                    nchar(text_clean) < 400) {
                                                  end_idx <- i - 1
                                            ...
                                            if (is.null(end_idx) || end_idx <= start_idx) {
                                              end_idx <- min(round(nrow(all_text) * 0.9), nrow(all_text))
                                              message(glue("No clear end found, using 90% cutoff at row {end_idx}"))
                                            }
                                            
                                            # Extract content with bounds checking
                                            start_idx <- max(1, start_idx)
                                            end_idx <- min(end_idx, nrow(all_text))
                                            
                                            if (start_idx > end_idx) {
                                              message("WARNING: Invalid boundaries, using full text")
                                              start_idx <- 1
                                            ...
                                            # Before the final return, add deduplication:
                                            main_content <- main_content %>%
                                              # Remove duplicated sentences
                                              str_split("\\. ") %>%
                                              map(~ unique(.x)) %>%  # Remove duplicates
                                              map_chr(~ paste(.x, collapse = ". ")) %>%
                                              # Remove abbreviations sections
                                              str_replace_all("ABBREVIATIONS.*?INTRODUCTION", "INTRODUCTION") %>%
...
summary2 <- processor$process_document_simple(pdf, cleaning_method = "hybrid")
claude_text_cleaned2 <- processor$prepare_for_claude(summary2, body_only = TRUE, 
                                                     cleaning_method = "cleaned")
